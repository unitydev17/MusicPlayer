...
public static class OpenWavParser
{
	...
	public static AudioModel ByteArrayToAudioClipModel(byte[] wavFile, string name = "", bool stream = false)
    {
        /* WAV file format:
         * 
        ...
        if (_audioFormat == 1)
        {
			...
			model.name = "temp";
			model.lengthSamples = _sampleCount;
			model.channel = _numChannels;
			model.frequency = _sampleRate;
			model.stream = stream;
			model.data = _audioBuffer;

			return model;
        }
        else
        {
			throw new UnityException("[OpenWavParser.ByteArrayToAudioClip] Compressed wav format not supported.");
        }
    }
